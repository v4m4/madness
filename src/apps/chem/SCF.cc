    ...
    void SCF::polarizability(World & world)
    {
        ...
        if (xc.is_dft()) {
            ...
            if (xc.is_gga()) {
                    ...
                    vf.push_back(   delrho[0] * delrho[0] 
                                  + delrho[1] * delrho[1]
                   ...
                   // if (xc.is_spin_polarized() && param.nbeta != 0)
                    if (!param.spin_restricted && param.nbeta != 0)
                    {
                           vf.push_back(  delrho[0] * delrho[3] 
                                        + delrho[1] * delrho[4]
                                        + delrho[2] * delrho[5]); // sigma_ab

                           vf.push_back(  delrho[3] * delrho[3] 
                                        + delrho[4] * delrho[4]
            ...
            //  has to be here or refine_to_common_level(vf) above hangs, but we really need a 
            //  better solution for when nbeta=0
            if (vf.size()) {
...
//efield
            ...
            if ( param.efield != 0.0) {
                std::vector<int> f(3, 0);
                f[param.efield_axis] = true; // 2 -> axis = z
                functionT dipolefunc = factoryT(world).functor(functorT(new MomentFunctor(f)));
                dipolefunc.scale(param.efield);
                dipolefunc.truncate();
// adding dipole term to the hamiltonian
                double edipole = inner(rho, dipolefunc);
                print("Energy of finite field = ", edipole);

                vlocal = vlocal - dipolefunc;
            }
		...
		if (xc.is_spin_polarized() && param.nbeta == 0 && xc.is_gga()){
            ...
            double etot = ekinetic + enuclear + ecoulomb + exc + enrep + enonlocal + edipole;
            current_energy = etot;
            //esol = etot;
